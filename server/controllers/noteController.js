...
const getNotes = asyncHandler(async (req, res) => {
  ...
  const formattedNotes = notes.map((note) => ({
    ...note.toObject(),
    authorUsername: note.uploadedBy.userName,
  }));
...
const getNoteById = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, { ...note.toObject(), authorUsername: note.uploadedBy.userName }, 'Note fetched successfully'));
});
