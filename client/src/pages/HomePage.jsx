...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const notesResponse = await getNotes({ isFeatured: true });
        setNotes(notesResponse.data);
      } catch (err) {
        setError(err.message || 'Failed to load featured notes');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a23] via-[#12122b] to-black text-white px-6 py-12">
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold mb-6 text-left">Featured Notes</h2>
